...
Build the local Conda environment (or link an existing one) and activate it:
```sh
# Option A: Build local conda environment
make env
source env/conda/bin/activate cfu-common

# Option B: Link and activate an existing CFU-Playground conda environment
source ~/cfu-playground-fork/CFU-Playground/env/conda/bin/activate cfu-common
ln -s ~/cfu-playground-fork/CFU-Playground/env env
```

Verify that the toolchain is active in your PATH:
```sh
which riscv32-unknown-elf-gcc
```

> [!CAUTION]
> **Do not run `make clean` at the repository root!**
> Running `make clean` in the root folder removes `env/conda`. To clean project builds, always run `make clean` from inside the specific project directory (`proj/<project_name>`).
...
> [!IMPORTANT]
> Simulation and build targets like `renode` and `load` must be executed **inside a project directory** (e.g. `proj/mnv2_first`, `proj/proj_template`), not from the repository root.

#### Renode Simulation

Navigate to your target project folder and run Renode:

```sh
cd proj/mnv2_first   # or proj/proj_template
make renode
```

For headless environments (terminal only / SSH without X11):
```sh
...
To use Verilator to execute on a cycle-accurate RTL-level simulator (no Vivado or Arty board required), navigate to the project directory and execute:
